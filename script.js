...
import { initI18n, setLanguage, applyTranslations, translations, getTranslatedText } from './i18n.js';
...
function renderWelcome(msg) {
  ...
  // إيقاف أي تأثير كتابة سابق فوراً
  ...
  // بدء تأثير الكتابة الجديد بعد تأخير بسيط
  setTimeout(() => {
    initTypingEffect();
  }, 100);
}
...
function initTypingEffect() {
  const container = document.getElementById('typing-container');
  if (!container || !welcomeMessage) return;
  ...
  function calculateLines(text) {
    ...
    // استخدام عنصر قياس فعلي للحصول على العرض الدقيق
    const measureElement = document.createElement('span');
    measureElement.style.visibility = 'hidden';
    measureElement.style.position = 'absolute';
    measureElement.style.whiteSpace = 'nowrap';
    measureElement.style.font = getComputedStyle(container).font;
    measureElement.style.width = 'auto';
    measureElement.style.padding = '0 10px'; // إضافة padding لمطابقة التصميم الفعلي
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      // استخدام 90% من العرض للسماح بهامش آمن
      if (measureElement.offsetWidth > container.clientWidth * 0.90) {
        if (currentLine === '') {
          // إذا كانت الكلمة وحدها أطول من العرض، نضطر لوضعها في سطر
          lines.push(words[i]);
          currentLine = '';
        } else {
          // إضافة السطر الحالي وبدء سطر جديد بالكلمة الحالية
    ...
    console.log('Lines calculated:', lines); // للتتبع
    return lines;
  }
  ...
  if (lines.length === 0) {
    container.innerHTML = '<div class="typing-line">' + welcomeMessage + '</div>';
    return;
  }
  ...
  function typeNextLine() {
    ...
    function typeCharacter() {
      // التأكد من أن الحاوية لا تزال موجودة
      if (!document.body.contains(container)) {
        return;
      }
      
      if (charIndex <= currentLine.length) {
        lineDiv.innerHTML = currentLine.substring(0, charIndex) + '<span class="blinking-cursor">|</span>';
        charIndex++;
        typingTimer = setTimeout(typeCharacter, 80);
      } else {
        // إزالة المؤشر عند اكتمال السطر
        lineDiv.innerHTML = currentLine;
        currentLineIndex++;
        
        // الانتقال للسطر التالي بعد فترة
        typingTimer = setTimeout(typeNextLine, 500);
      }
    }
...
async function sendOrder(name, phone, message, location, paymentMethod, paymentData = null) {
  ...
  try {
    ...
    const phoneNumber = phoneInput ? phoneInput.value.trim() : phone;
    
    const cleanNumber = phoneNumber.replace(/\D/g, '');
    let finalNumber = cleanNumber.replace(/^0+/, '');
    const fullPhone = countryCode + finalNumber;
    ...
    console.log('جاري حفظ الطلب:', orderData);
    ...
    // دائماً تأكد من إعادة تعيين حالة الإرسال
    isSubmitting = false;
    
    const submitBtn = document.getElementById('confirm-checkout');
    if (submitBtn) {
      submitBtn.disabled = false;
      const paymentMethod = document.querySelector('.payment-option.selected')?.dataset.method || 'cash';
...
function performSearch() {
    try {
        ...
        initAllCarousels();
        console.log('Search completed. Results:', Object.keys(filtered).length);
    } catch (error) {
...
function initDraggableCart() {
  ...
  let isDragging = false; // تعريف المتغير محلياً داخل الدالة
  ...
  function saveCartIconPosition(icon) {
    ...
    console.log('Cart position saved:', pos); 
  }
  ...
  // إرجاع دالة constrainCartPosition للاستخدام الخارجي
  return {
    constrainCartPosition: () => constrainCartPosition(cartIcon),
...
// ==================== نظام تتبع الطلبات ====================
// ==================== نظام تتبع الطلبات ====================
let currentTrackingSource = 'main'; // 'main' أو 'search'

/**
 * تهيئة نموذج تتبع الطلبات والأحداث المرتبطة به
 */
function initOrderTrackingModal() {
    ...
    if (searchOrderBtn) {
        searchOrderBtn.addEventListener('click', openOrderSearch);
    }

    if (searchOrdersForm) {
        searchOrdersForm.addEventListener('submit', handleOrderSearch);
    }
    ...
    // تحميل رموز الدول في نموذج البحث
    loadCountryCodes('search-order-country-code', '20');
}

/**
 * فتح نموذج تتبع الطلبات
 */
function openTrackingModal() {
    const trackingModal = document.getElementById('order-tracking-modal');
    const overlay = document.getElementById('overlay');
    
    if (trackingModal && overlay) {
...
/**
 * إعادة تعيين النموذج إلى الحالة الافتراضية
 */
function resetTrackingModal() {
    currentTrackingView = 'main';
    currentTrackingSource = 'main';
    updateModalView();
}

/**
 * تحديث واجهة النموذج بناءً على العرض الحالي
 */
function updateModalView() {
    ...
    const backButton = document.getElementById('back-button');
    const titleElement = document.getElementById('tracking-modal-title');
    const footer = document.querySelector('.modal-footer');
    
    // تحديث الواجهة بناءً على العرض الحالي
    switch(currentTrackingView) {
        case 'main':
            document.getElementById('my-orders-content').classList.remove('hidden');
            if (backButton) backButton.classList.add('hidden');
            if (titleElement) titleElement.textContent = getTranslatedText('my_orders');
            if (footer) footer.classList.remove('hidden');
            break;
            
        case 'search':
            document.getElementById('order-search-content').classList.remove('hidden');
            if (backButton) backButton.classList.remove('hidden');
            if (titleElement) titleElement.textContent = getTranslatedText('search_order');
        ...
        case 'search-results':
            ...
            if (backButton) backButton.classList.remove('hidden');
            if (titleElement) titleElement.textContent = getTranslatedText('search_results');
            if (footer) footer.classList.add('hidden');
            break;
        ...
        case 'details':
            ...
            if (backButton) backButton.classList.remove('hidden');
            if (titleElement) titleElement.textContent = getTranslatedText('details_o');
            if (footer) footer.classList.add('hidden');
            break;
    }
...
window.handleBackButton = function() {
    switch(currentTrackingView) {
        ...
        case 'search-results':
            currentTrackingView = 'search';
            updateModalView();
            break;
            
        case 'details':
            // الرجوع إلى العرض السابق بناءً على المصدر
            if (currentTrackingSource === 'search') {
                currentTrackingView = 'search-results';
            } else {
                currentTrackingView = 'main';
            }
            updateModalView();
...
async function handleOrderSearch(event) {
    ...
    const name = document.getElementById('search-order-name').value.trim();
    const countryCode = document.getElementById('search-order-country-code').value;
    const phone = document.getElementById('search-order-phone').value.trim();

    // التحقق من صحة البيانات
    if (!name) {
        showToast(getTranslatedText('required_name'), 'error');
        return;
    }

    if (!phone) {
        showToast(getTranslatedText('required_phone'), 'error');
        return;
    }
        ...
        if (searchResults) {
            searchResults.innerHTML = `
                <div class="tracking-loading">
                    ...
                    <p>جاري البحث عن الطلبات...</p>
                </div>
            `;
        }
        
        // الانتقال إلى عرض نتائج البحث أولاً
        currentTrackingView = 'search-results';
        updateModalView();
        
        // تنظيف رقم الهاتف وإعداد البحث
        const cleanPhone = phone.replace(/\D/g, '');
        const fullPhone = countryCode + cleanPhone.replace(/^0+/, '');
        
        // ثم تنفيذ البحث وعرض النتائج
        const orders = await searchOrdersByCustomer(name, fullPhone);
        currentSearchResults = orders; // تخزين نتائج البحث
        ...
        if (searchResults) {
            searchResults.innerHTML = `
                <div class="tracking-not-found">
                    ...
                    <h3>خطأ في البحث</h3>
                    <p>حدث خطأ أثناء البحث عن الطلبات. الرجاء المحاولة مرة أخرى.</p>
                </div>
            `;
        }
    }
...
/**
 * البحث عن الطلبات بناءً على اسم العميل ورقم الهاتف
 */
async function searchOrdersByCustomer(name, phone) {
    return new Promise((resolve, reject) => {
        const ordersRef = ref(db, 'storeOrders');
            ...
            if (ordersData) {
                Object.entries(ordersData).forEach(([key, order]) => {
                    ...
                    if (order && order.name && order.phone) {
                        ...
                        const nameMatch = orderName.includes(searchName);
                        const phoneMatch = orderPhone.includes(searchPhone);
...
function displaySearchResults(orders) {
    const resultsPage = document.getElementById('search-results-content');
    if (!resultsPage) return;
    
    // التأكد من أن orders مصفوفة صالحة
    ...
    if (orders.length === 0) {
        resultsPage.innerHTML = `
            <div class="tracking-not-found">
                ...
                <h3>${getTranslatedText('no_search_results')}</h3>
                <p>${getTranslatedText('no_orders_message')}</p>
            </div>
        `;
    } else {
        resultsPage.innerHTML = `
            <div class="search-results-header">
                <h4>${getTranslatedText('search_results')} (${orders.length})</h4>
            </div>
            <div class="orders-list" id="search-results-list">
                ${orders.map((order) => {
                    // التحقق من وجود order وخصائصها الأساسية
                    if (!order) return '';
                    
                    const displayStatus = order.status === 'paid' ? 'confirmed' : (order.status || 'pending');
                    const formattedDate = formatDateShort(order.timestamp);
                    const orderName = order.name || 'غير معروف';
                    const orderPhone = order.phone || '';
                    const orderLocation = order.location || getTranslatedText('location_not_specified');
                    const productsCount = order.products && Array.isArray(order.products) ? order.products.length : 0;
                    const orderTotal = order.total || 0;
                    const orderCurrency = order.currency || getTranslatedText('currency');
                    
                    return `
                    <div class="order-item" data-order-id="${order.id}" onclick="selectOrderFromSearch('${order.id}')">
                        <div class="order-main-info">
                            <div class="info-row">
                                <div class="info-item">
                                    <i class="fas fa-user"></i>
                                    <span class="label">${getTranslatedText('customer_name_display')}:</span>
                                    <span class="value">${orderName}</span>
                                </div>
                                <div class="info-item">
                                    <i class="fas fa-phone"></i>
                                    <span class="label">${getTranslatedText('customer_phone_display')}:</span>
                                    <span class="value">${formatPhoneNumberDisplay(orderPhone)}</span>
                                </div>
                            </div>
                            ...
                            <div class="info-row">
                                <div class="info-item full-width">
                                    ...
                                    <span class="label">${getTranslatedText('order_location_display')}:</span>
                                    <span class="value">${orderLocation}</span>
                                </div>
                            </div>
                            ...
                            <div class="info-row">
                                <div class="info-item">
                                    ...
                                    <span class="label">${getTranslatedText('order_date_short')}:</span>
                                    <span class="value">${formattedDate}</span>
                                </div>
                                <div class="info-item">
                                    <i class="fas fa-cube"></i>
                                    <span class="label">${getTranslatedText('products_count_display')}:</span>
                            ...
                            <div class="info-row">
                                <div class="info-item">
                                    ...
                                    <span class="label">${getTranslatedText('order_total_display')}:</span>
                                    <span class="value">${orderTotal} ${orderCurrency}</span>
                                </div>
                                <div class="info-item status-item">
                                    <div class="order-status ${displayStatus}">
                                        <i class="fas ${getStatusIcon(displayStatus)}"></i>
...
/**
 * تحديد طلب من نتائج البحث وعرض تفاصيله - الإصدار المُصلح
 */
window.selectOrderFromSearch = function(orderId) {
    // البحث عن الطلب في نتائج البحث الحالية
    const order = currentSearchResults.find(o => o.id === orderId);
    
    if (!order) {
        console.error('Order not found in search results:', orderId);
    ...
    const selectedElement = document.querySelector(`.order-item[data-order-id="${orderId}"]`);
    if (selectedElement) {
        selectedElement.classList.add('active');
    }
    
    // تعيين المصدر إلى 'search' للرجوع إلى نتائج البحث
    currentTrackingSource = 'search';
...
async function loadMyOrders() {
    ...
    if (noOrdersFound) noOrdersFound.classList.add('hidden');
    ...
    try {
        ...
        if (orders && orders.length > 0) {
            ...
            displayMyOrdersList(orders);
            if (ordersList) ordersList.classList.remove('hidden');
        } else {
            if (noOrdersFound) noOrdersFound.classList.remove('hidden');
        }
    } catch (error) {
...
/**
 * الحصول على الطلبات الخاصة بالمستخدم من قاعدة البيانات
 */
async function getMyOrders(userId) {
    return new Promise((resolve, reject) => {
        ...
        onValue(ordersRef, (snapshot) => {
            ...
            if (ordersData) {
                Object.entries(ordersData).forEach(([key, order]) => {
                    const orderUserId = order.userId || order.userUniqueId;
 ...
 */
function displayMyOrdersList(orders) {
    const ordersList = document.getElementById('orders-list');
    const noOrdersFound = document.getElementById('no-orders-found');
    ...
    ordersList.innerHTML = orders.map((order) => {
        const displayStatus = order.status === 'paid' ? 'confirmed' : order.status;
        const formattedDate = formatDateShort(order.timestamp);
        const productsCount = order.products && Array.isArray(order.products) ? order.products.length : 0;
            ...
            <div class="order-main-info">
                <div class="info-row">
                    <div class="info-item">
                        <i class="fas fa-user"></i>
                        <span class="label">${getTranslatedText('customer_name_display')}:</span>
                        <span class="value">${order.name || 'غير معروف'}</span>
                    </div>
                    <div class="info-item">
                        <i class="fas fa-phone"></i>
                        <span class="label">${getTranslatedText('customer_phone_display')}:</span>
                        <span class="value">${formatPhoneNumberDisplay(order.phone || '')}</span>
                    </div>
                </div>
                ...
                <div class="info-row">
                    <div class="info-item full-width">
                        <i class="fas fa-map-marker-alt"></i>
                        <span class="label">${getTranslatedText('order_location_display')}:</span>
                        <span class="value">${order.location || getTranslatedText('location_not_specified')}</span>
                    </div>
                </div>
                ...
                <div class="info-row">
                    <div class="info-item">
                        <i class="fas fa-receipt"></i>
                        <span class="label">${getTranslatedText('order_total_display')}:</span>
                        <span class="value">${order.total || 0} ${order.currency || getTranslatedText('currency')}</span>
                    </div>
                    <div class="info-item status-item">
                        <div class="order-status ${displayStatus}">
                            <i class="fas ${getStatusIcon(displayStatus)}"></i>
    ...
    // إضافة event listeners لعرض التفاصيل
    ordersList.querySelectorAll('.order-item').forEach(item => {
        const orderId = item.dataset.orderId;
        const mainContent = item.querySelector('.order-main-info');
...
/**
 * تحديد طلب من القائمة الرئيسية وعرض تفاصيله - الإصدار المُصلح
 */
window.selectOrder = function(orderId) {
    // البحث عن الطلب في الطلبات العادية
    ...
    // تعيين المصدر إلى 'main' للرجوع إلى الطلبيات العادية
    currentTrackingSource = 'main';
...
/**
 * عرض تفاصيل الطلب المحدد - الإصدار المُصلح بالكامل
 */
function showOrderDetails(order) {
    const detailsContent = document.getElementById('order-details-content');
    if (!detailsContent) return;
    
    // التحقق من وجود البيانات الأساسية للطلب
    if (!order) {
        detailsContent.innerHTML = `
            <div class="tracking-not-found">
                ...
                <p>تعذر تحميل تفاصيل الطلب. الرجاء المحاولة مرة أخرى.</p>
            </div>
        `;
        return;
    }

    const orderDateShort = formatDateShort(order.timestamp);
    const estimatedDelivery = order.estimatedDelivery ? formatDateShort(order.estimatedDelivery) : getTranslatedText('location_not_specified');
    const productsCount = order.products && Array.isArray(order.products) ? order.products.length : 0;
    const orderName = order.name || 'غير معروف';
    const orderPhone = order.phone || '';
    const orderLocation = order.location || getTranslatedText('location_not_specified');
    const orderTotal = order.total || 0;
    const orderCurrency = order.currency || getTranslatedText('currency');
    const orderId = order.orderId || 'غير متوفر';
    const paymentMethod = order.paymentMethod === 'cash' ? 
        getTranslatedText('payment_cash') : 
        getTranslatedText('payment_method_card');

    detailsContent.innerHTML = `
        <div class="order-main-info">
            <div class="info-card">
                <h4>${getTranslatedText('order_info')}</h4>
                <div class="info-grid">
                    <div class="info-row">
                        <span class="info-label">${getTranslatedText('order_id')}:</span>
                        <span class="info-value">${orderId}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">${getTranslatedText('order_date')}:</span>
                        <span class="info-value">${orderDateShort}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">${getTranslatedText('estimated_delivery')}:</span>
                        <span class="info-value">${estimatedDelivery}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">${getTranslatedText('order_status')}:</span>
                        <span class="info-value status-${order.status}">
                            ${getOrderStatusText(order.status)}
                        </span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">${getTranslatedText('payment_method')}:</span>
                        <span class="info-value">${paymentMethod}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">${getTranslatedText('order_total')}:</span>
                        <span class="info-value">${orderTotal} ${orderCurrency}</span>
                    </div>
                </div>
            </div>
            
            <div class="info-card">
                <h4>${getTranslatedText('customer_info')}</h4>
                <div class="info-grid">
                    <div class="info-row">
                        <span class="info-label">${getTranslatedText('customer_name')}:</span>
                        <span class="info-value">${orderName}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">${getTranslatedText('customer_phone')}:</span>
                        <span class="info-value">${formatPhoneNumberDisplay(orderPhone)}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">${getTranslatedText('customer_address')}:</span>
                        <span class="info-value">${orderLocation}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="order-products-section">
            <h4>${getTranslatedText('order_products')} (${productsCount})</h4>
            <div class="products-grid">
                ${order.products && Array.isArray(order.products) ? 
                    order.products.map(product => `
                        <div class="product-card">
                            <div class="product-image">
                                <img src="${product.image || 'placeholder.png'}" alt="${product.title || 'منتج'}" loading="lazy">
                            </div>
                            <div class="product-details">
                                <h5 class="product-title">${product.title || 'منتج'}</h5>
                                <div class="product-meta">
                                    <span class="product-price">${product.price || 0} ${orderCurrency}</span>
                                    <span class="product-quantity">${getTranslatedText('product_quantity')}: ${product.quantity || 0}</span>
                                    <span class="product-subtotal">${getTranslatedText('product_subtotal')}: ${product.subtotal || 0} ${orderCurrency}</span>
                                </div>
                            </div>
                        </div>
                    `).join('') : 
        ...
        <div class="order-timeline-section">
            <h4>${getTranslatedText('order_timeline')}</h4>
    ...
    // الانتقال إلى عرض التفاصيل
    currentTrackingView = 'details';
    updateModalView();
...
/**
 * عرض الخط الزمني للطلب
 */
function renderTimelineSteps(order) {
    ...
    const steps = [
        { 
            key: 'ordered', 
            time: timelineData.ordered,
            label: getTranslatedText('timeline_ordered'),
            icon: 'fas fa-shopping-cart'
        },
        { 
            key: 'confirmed', 
            time: timelineData.confirmed,
            label: getTranslatedText('timeline_confirmed'),
            icon: 'fas fa-check-circle'
        },
        { 
            key: 'processing', 
            time: timelineData.processing,
            label: getTranslatedText('timeline_processing'),
            icon: 'fas fa-cogs'
        },
        { 
            key: 'shipped', 
            time: timelineData.shipped,
            label: getTranslatedText('timeline_shipped'),
            icon: 'fas fa-shipping-fast'
        },
        { 
            key: 'delivered', 
            time: timelineData.delivered,
            label: getTranslatedText('timeline_delivered'),
            icon: 'fas fa-home'
        }
    ];
    
    // إضافة خطوة الإلغاء إذا كان الطلب ملغى
    if (status === 'cancelled' && timelineData.cancelled) {
        steps.push({
            key: 'cancelled',
            time: timelineData.cancelled,
            label: getTranslatedText('timeline_cancelled'),
    ...
    switch(status) {
        ...
        case 'cancelled': currentStep = steps.length - 1; break;
        default: currentStep = 0;
    }
    
    return steps.map((step, index) => {
        const stepClass = index < currentStep ? 'completed' : 
                         index === currentStep ? 'current' : 'upcoming';
        const stepTime = step.time ? formatDateShort(step.time) : '';
        ...
        return `
            <div class="timeline-step ${stepClass}" 
                ...
                <div class="timeline-content">
                    <h5>${step.label}</h5>
                    ${stepTime ? `<span class="timeline-time">${stepTime}</span>` : ''}
                </div>
            </div>
        `;
    }).join('');
...
function renderCancelButton(order) {
    ...
    if (isCancelled) {
        return `
            <button class="cancel-order-btn cancelled" disabled>
                <i class="fas fa-ban"></i>
                <span>${getTranslatedText('order_cancelled')}</span>
            </button>
        `;
    }
    
    const cancelText = getTranslatedText('cancel_order');
    const disabledText = getTranslatedText('contact_support_to_cancel');
    ...
    if (canCancel) {
        ...
        return `
            <button class="cancel-order-btn disabled" 
...
async function cancelOrder(orderId) {
    ...
    const lang = currentLang();
    const confirmMsg = translations['confirm_cancel_order']?.[lang] || 
    ...
    try {
        showToast('جاري إلغاء الطلب...', 'info');
        
        const orderRef = ref(db, `storeOrders/${orderId}`);
        ...
        // تحديث الواجهة
        const activeOrderElement = document.querySelector('.order-item.active');
        if (activeOrderElement) {
            updateOrderStatusInUI(activeOrderElement, 'cancelled');
        }
        ...
        // إعادة تحميل القوائم بعد ثانية
        setTimeout(() => {
            if (currentTrackingView === 'main') {
                loadMyOrders();
            } else if (currentTrackingView === 'search-results') {
                const name = document.getElementById('search-order-name')?.value;
                const phone = document.getElementById('search-order-phone')?.value;
                if (name && phone) {
                    handleOrderSearch(new Event('submit'));
                }
            }
        }, 1000);
        ...
        const errorMsg = translations['order_cancel_failed']?.[currentLang()] || 'فشل في إلغاء الطلب';
        showToast(errorMsg, 'error');
    }
}
 ...
 */
function updateOrderStatusInUI(orderElement, newStatus) {
    ...
    if (statusElement) {
        ...
        statusElement.innerHTML = `
            ...
            <span>${getOrderStatusText(newStatus)}</span>
        `;
    }
    
    updateCancelButtonInDetails(newStatus);
}

/**
 * تحديث زر الإلغاء في عرض التفاصيل
 */
function updateCancelButtonInDetails(status) {
    ...
    if (isCancelled) {
        ...
        cancelBtn.classList.remove('cancelled', 'disabled');
        const orderId = document.querySelector('.order-item.active')?.dataset.orderId;
        if (orderId) {
            cancelBtn.onclick = () => handleCancelOrder(orderId);
        }
    } else {
        cancelBtn.disabled = true;
        cancelBtn.classList.add('disabled');
        cancelBtn.classList.remove('cancelled');
    }
}

/**
 * عرض رسالة تنبيه عند الضغط على زر الإلغاء المعطل
 */
window.showCancelAlert = function() {
    const lang = currentLang();
    const alertMsg = translations['contact_support_to_cancel']?.[lang] || 'تواصل مع خدمة العملاء للإلغاء';
...
function formatPhoneNumberDisplay(phone) {
    if (!phone) return '';
    const cleanNumber = phone.replace(/^\+\d+/, '');
    return cleanNumber;
}
...
window.backToOrdersList = function() {
    currentTrackingView = 'main';
    updateModalView();
};


// ==================== إدارة التمرير للنوافذ المنبثقة ====================
class ScrollManager {
    constructor() {
    ...
    setupModalListeners() {
        // مراقبة فتح وإغلاق جميع النوافذ المنبثقة
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    this.handleModalState(mutation.target);
                }
            });
        });

        // مراقبة جميع العناصر التي تحتوي على نوافذ منبثقة
        const modalElements = [
            document.getElementById('order-tracking-modal'),
            document.getElementById('cart-popup'),
            document.getElementById('checkout-form-popup')
        ];
    ...
    handleModalState(element) {
        const isNowVisible = !element.classList.contains('hidden');
        
        if (isNowVisible && !this.isModalOpen) {
            this.disableBodyScroll();
        } else if (!isNowVisible && this.isModalOpen) {
    ...
    disableBodyScroll() {
        ...
        // إعادة تعيين تمرير النوافذ المنبثقة
        setTimeout(resetModalScroll, 100);
    }

    

    // دالة للتحقق مما إذا كان أي نافذة منبثقة مفتوحة
    isAnyModalOpen() {
        const modals = [
...
// تهيئة مدير التمرير عند تحميل الصفحة
let scrollManager;
...
function setupEnhancedScrollHandling() {
    // منع تمرير body عند التمرير داخل النوافذ المنبثقة
    document.addEventListener('wheel', function(e) {
        const activeModal = document.querySelector('.modal:not(.hidden), #cart-popup:not(.hidden), #checkout-form-popup:not(.hidden)');
        
        if (activeModal) {
            const isScrollable = activeModal.scrollHeight > activeModal.clientHeight;
            const isAtTop = activeModal.scrollTop === 0;
            const isAtBottom = activeModal.scrollTop + activeModal.clientHeight >= activeModal.scrollHeight;
            
            // إذا لم يكن العنصر قابلاً للتمرير، أو وصلنا للنهايات، منع التمرير الإضافي
    ...
    // تحسين التعامل مع اللمس للأجهزة المحمولة
    ...
    // منع تمرير body عند استخدام مفاتيح الأسهم
    document.addEventListener('keydown', function(e) {
        if (scrollManager.isAnyModalOpen() && 
...
// إضافة تحسينات التمرير عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', function() {
    setupEnhancedScrollHandling();
});

// ==================== حل خاص للنوافذ المنبثقة المختلفة ====================
function setupIndividualModalScrollHandling() {
    // للنافذة الرئيسية لتتبع الطلبات
    const trackingModal = document.getElementById('order-tracking-modal');
    if (trackingModal) {
        trackingModal.addEventListener('wheel', function(e) {
    ...
    const cartPopup = document.getElementById('cart-popup');
    if (cartPopup) {
        cartPopup.addEventListener('wheel', function(e) {
            const cartItems = this.querySelector('.cart-items');
            if (cartItems) {
                const isScrollable = cartItems.scrollHeight > cartItems.clientHeight;
                const isAtTop = cartItems.scrollTop === 0;
                const isAtBottom = cartItems.scrollTop + cartItems.clientHeight >= cartItems.scrollHeight;
    ...
    const checkoutPopup = document.getElementById('checkout-form-popup');
    if (checkoutPopup) {
        checkoutPopup.addEventListener('wheel', function(e) {
            const checkoutForm = this.querySelector('#checkout-form');
            if (checkoutForm) {
                const isScrollable = checkoutForm.scrollHeight > checkoutForm.clientHeight;
                const isAtTop = checkoutForm.scrollTop === 0;
                const isAtBottom = checkoutForm.scrollTop + checkoutForm.clientHeight >= checkoutForm.scrollHeight;
...
// تهيئة التحكم الفردي في التمرير
document.addEventListener('DOMContentLoaded', function() {
    setupIndividualModalScrollHandling();
});
/**
 * إعادة تعيين التمرير لجميع محتويات النافذة المنبثقة
 */
function resetModalScroll() {
    const modalContents = [
        document.getElementById('my-orders-content'),
        document.getElementById('order-search-content'),
...
window.selectOrderFromSearch = selectOrderFromSearch;
window.handleCancelOrder = handleCancelOrder;
window.showCancelAlert = showCancelAlert;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.addEventListener('languageChanged', async () => {
    ...
    if (currentUserOrders.length > 0) {
      displayMyOrdersList(currentUserOrders);
      // إذا كان هناك طلب معروض حاليًا، قم بتحديثه أيضًا
      const activeOrder = document.querySelector('.order-item.active');
      if (activeOrder) {
        const orderId = activeOrder.dataset.orderId;
  ...
  if (currentPromo?.welcomeMessage) {
      renderWelcome(currentPromo.welcomeMessage);
  }

const draggableCart = initDraggableCart();
